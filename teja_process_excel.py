...
def count_word_in_column(file_path: pathlib.Path, column_letter: str, word: str) -> int:
    ...
    try:
        sales =  pd.read_excel(file_path,header=0)
        word_count = sales[column_letter].str.contains('LLC', case=False, na=False).sum()
...
def process_excel_file():
    ...
    input_file = pathlib.Path(FETCHED_DATA_DIR, "sample_sales.xlsx")

    output_file = pathlib.Path(PROCESSED_DIR, "excel_company_llc_count.txt")

    column_to_check = "name"  #modifies this to match the column header
